...
export function connectStudySpaceSocket(
  options:
    StudySpaceSocketOptions,
): () => void {
  let heartbeatTimer:
    number | null =
    null;
    ...
    new Client({
        ...
        sessionSubscription =
          client.subscribe(
            ...
            (
              ...
              options.onSessionTick(
                parseMessage<SessionTick>(
                  message,
                ),
              ),
          );
      ...
      onWebSocketError:
        () => {
          ...
          options.onError(
            "실시간 좌석 서버에 연결할 수 없습니다. REST 방식으로 연결을 유지합니다.",
          );
        },
    });
