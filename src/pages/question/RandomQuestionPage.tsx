...
import { getRandomQuestions, solveQuestion } from "@/api/questionApi";
import type { QuestionDetailResponse, SolveResultResponse } from "@/api/types";
...
export default function RandomQuestionPage() {
  ...
  async function handleStart() {
    ...
    setError(null);
    try {
      const qs = await getRandomQuestions({ count });
      setQuestions(qs);
      setPage(0);
      setSelected({});
      setResults({});
  ...
  async function handleSelect(q: QuestionDetailResponse, optionId: number) {
    ...
    try {
      const result = await solveQuestion(q.questionId, optionId, "RANDOM");
